...
estimate \\(\Theta = \(\theta_A, \theta_B\)\\), we can divide the experiment into n repeated runs of the following procedure: (1) randomly choose one of the two coins;
(2) and perform m independent coin tosses with the selected coin. In other words, we keep track of two vectors \\( x = \(x_1, x_2, x_3, ..., x_n\) \\) and 
\\( z = \(z_1, z_2, z_3, ..., z_n\) \\) where \\( x_i \in {0, 1, 2, ..., m} \\) is the number of heads observed during the ith set of tosses, and \\( z_i \in {A, B} \\)
