...
In the case of K-means, similarly, after each cluster \\( z \\) is assigned with a center \\( \mu^z \\), we repeat the following two steps: (1) decide \\( z_t \\) for each \\( x_t \\) given \\( (\mu^1, \mu^2, ..., \mu^K) \\), i.e, find \\( z_t \\) so that \\( x_t \\) is closest to \\( \mu^{z_t} \\); (2) recompute \\( \mu^1, \mu^2, ..., \mu^K \\) given the assignment of \\( z_t \\) of each \\( x_t \\). Given the pdf of Gaussian, one can easily show that it is equivalent to the following,
